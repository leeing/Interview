 ...
 *
 * 这个 Control 调用 protocol 上一个特定的方法。这个方法是由 config 参数(PAR_METHOD),
 * 而 protocol 是 #PAR_PROT 来定义的。这个方法不可以拥有任何参数并且必须返回void。如果
 * 没有指定protocol，那这个方法会在 protocol stack上的所有定义了它的protocol上调用。
 *
 * 尽管这个方法不能拥有任何参数，但它能读取 CommonState，它能保证状态是最新的，包括eg方法。
 */
